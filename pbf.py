...
bd = (40, 20, 20)
boundary = (bd[0]/screen_to_world_ratio, bd[1] /
            screen_to_world_ratio, bd[2]/screen_to_world_ratio)
...
grid_size = (round_up(boundary[0], 1), round_up(
    boundary[1], 1), round_up(boundary[2], 1))
...
def confine_position_to_boundary(p):
    ...
    bmax = ti.Vector([board_states[None][0], boundary[1],
                      boundary[2]]) - particle_radius_in_world
...
def move_board(value: float):
    ...
    b[0] += -ti.sin(b[1] * np.pi / period) * value * time_delta
    board_states[None] = b
...
def compute_lambdas():
    ...
    for p_i in positions:
        ...
        lambdas[p_i] = (-density_constraint) / \
            (sum_gradient_sqr + lambda_epsilon)
...
def init_particles():
    ...
    for i in range(num_particles):
        np_positions[i] = np.array([i % num_x, i // (num_x * num_z), (i - (
            (i // (num_x * num_z)) * (num_x * num_z))) // num_x]) * delta + offs
